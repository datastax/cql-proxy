...
type clWrapper struct {
	primitive.ConsistencyLevel
}

func (c *clWrapper) UnmarshalText(text []byte) error {
	switch strings.ToLower(string(text)) {
	case "any":
		c.ConsistencyLevel = primitive.ConsistencyLevelAny
	case "one":
		c.ConsistencyLevel = primitive.ConsistencyLevelOne
	case "two":
		c.ConsistencyLevel = primitive.ConsistencyLevelTwo
	case "three":
		c.ConsistencyLevel = primitive.ConsistencyLevelThree
	case "quorum":
		c.ConsistencyLevel = primitive.ConsistencyLevelQuorum
	case "all":
		c.ConsistencyLevel = primitive.ConsistencyLevelAll
	case "local_quorum":
		c.ConsistencyLevel = primitive.ConsistencyLevelLocalQuorum
	case "each_quorum":
		c.ConsistencyLevel = primitive.ConsistencyLevelEachQuorum
	case "serial":
		c.ConsistencyLevel = primitive.ConsistencyLevelSerial
	case "local_serial":
		c.ConsistencyLevel = primitive.ConsistencyLevelLocalSerial
	case "local_one":
		c.ConsistencyLevel = primitive.ConsistencyLevelLocalOne
	default:
		return fmt.Errorf("invalid consistency level: %s", string(text))
	}
